...
// actual_value: Der tatsächliche Wert, der aus der Berechnung stammt.
// tolerance: Die Toleranz, innerhalb derer der tatsächliche Wert akzeptiert wird.


TEST(VECTOR, MySquareOfLength3df) {
Vector3df vector = {3.0, 4.0, 1.0};

EXPECT_NEAR(26.0, vector.square_of_length(), 0.00001);
}

TEST(VECTOR, MyLength3df) {
Vector3df vector = {0.0, 5.0, 0.0};
float length = vector.length();
...
TEST(VECTOR, MyNormalize) {
Vector2df vector = {-6.0, -8.0};
...
TEST(VECTOR, MyGetReflective) {
Vector3df vector = {1.0, 0.0, 0.0};
Vector3df normal = {0.0, 1.0, 0.0};

Vector3df reflectiv = vector.get_reflective(normal);

EXPECT_NEAR(1.0, reflectiv[0], 0.00001);
EXPECT_NEAR(0.0, reflectiv[1], 0.00001);
EXPECT_NEAR(0.0, reflectiv[2], 0.00001);
}

//  Winkel von 45 Grad zur X-Achse
TEST(VECTOR, MyAngle45) {
Vector2df vector{1.0f, 1.0f};

EXPECT_NEAR(PI / 4.0f, vector.angle(0, 1), 0.00001);
}


TEST(VECTOR, MySumsTwoVectors3df) {
Vector3df vector = {1.0, 2.0, 3.0};
Vector3df addend = {4.0, 5.0, 6.0};
Vector3df sum = vector + addend;

EXPECT_NEAR( 5.0, sum[0], 0.00001);
EXPECT_NEAR(7.0, sum[1], 0.00001);
EXPECT_NEAR( 9.0, sum[2], 0.00001);
}



TEST(VECTOR, MyScalarVectorProduct) {
Vector3df vector1 = {1.0, 2.0, 3.0};
Vector3df vector2 = {4.0, 5.0, 6.0};

float scalar = vector1 * vector2;

EXPECT_NEAR(32.0, scalar, 0.00001);
EXPECT_NEAR(1.0, vector1[0], 0.00001);
EXPECT_NEAR(2.0,  vector1[1], 0.00001);
EXPECT_NEAR(3.0, vector1[2], 0.00001);
EXPECT_NEAR(4.0, vector2[0], 0.00001);
EXPECT_NEAR(5.0,  vector2[1], 0.00001);
EXPECT_NEAR(6.0, vector2[2], 0.00001);
}


TEST(VECTOR, MyCrossVectorProduct) {
Vector3df vector1 = {1.0, 0.0, 0.0};
Vector3df vector2 = {0.0, 1.0, 0.0};
Vector3df cross = vector1.cross_product(vector2);
